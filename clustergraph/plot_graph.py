...
def plot_pie(
    ...
    if nb_edges is not None:
        edges = sorted(G.edges(data=True), key=lambda x: x[2].get("label", 0))[
            :nb_edges
        ]
        G.clear_edges()
    ...
    edge_labels = {
        (u, v): "{:.2f}".format(data[variable]) for u, v, data in G.edges(data=True)
    }
...
def plot_colored_graph(
    graph, nb_edges=None, variable="weight_plot", size_nodes=1000, random_state=None
):
    """_summary_
