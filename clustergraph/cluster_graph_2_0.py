...
from annexe_functions import get_corresponding_edges
...
class Cluster_graph() :
    ...
    def distances_clusters(self, normalize= True) :
        ...
        self.vertices, self.edges, nb_points_by_clusters = self.distance_clusters.compute_all_distances()
...
#-------------------------------------------------------------------------------------------------------------------------------
    ...
    def graph_add_edges(self, nb_edges = -1) :
        ...
        for i in range(nb_edges):
            self.graph.add_edge(self.edges[i][0], self.edges[i][1], label= self.edges[i][2])
        return self.graph
    ...
    Return the list of edges 
    """
    def normalize_edges_diameter(self, metric_clusters = None, distance_points = None,
                             data_preparation = None, parameters_metric_clusters = None) :
        
        self.list_diameters = self.distance_clusters.diameter_clusters(metric_clusters, distance_points, data_preparation, parameters_metric_clusters )
        ...
        self.graph.remove_edges_from(list(graph.edges()))
        self.normalize_max()
        return self.edges
    ...
    def personnalize_graph(self, vertices = False, nb_edges = -1, edges = None) :
        ...
        for i in range(nb_edges):
            self.my_graph.add_edge(corres_edges[i][0], corres_edges[i][1], label= corres_edges[i][2])
    ...
    """
    Functions which lets the user unified existing vertices into a new one which is the union
    Returns networkx graph with the new vertices and edges
    
    Parameters :
    - list_groups : list of group of vertices, each group of vertices is a list of integers [node_i, node_j, node_p]
    each group will become one vertice, numbered in the same order the groups are given
    - edges : list of edges we want to constraint to
    - nb_edges : among the potential edges, we will take this number from shortest to longest
    
    # Option to choose if we want to show all labels of edges on the graph ?
    # Sort all the group of vertices ? to avoid potential errors
    """
    def my_graph_group_clusters(self,list_groups, nb_edges = -1, edges = None) :
        self.my_graph = nx.Graph()

        # ADD WANTED VERTICES TO THE GRAPH 
        # GET THE CORRESPONDING EDGES ALSO
        nodes = []
        for i in range(len(list_groups)) :
            points_covered = []

            for j in range(len(list_groups[i])) :
                nodes.append(list_groups[i][j])
                points_covered.extend( self.graph.nodes[list_groups[i][j] ]['points_covered']  )


            self.my_graph.add_node(i+1, size = len(points_covered), points_covered = points_covered, 
                                   vertices_covered = list_groups[i])


        # 1) get the corresponding edges
        corres_edges, distances_group, = good_edges_groups(list_groups, self.edges)

        # 2) adding those edges to the graph
        for distance in distances_group :
            self.my_graph.add_edge(distance[0],distance[1], label = distance[2], Min = distance[3],
                                  Max = distance[4])
    ...
    # Return a list of the maximum of distortion for each nb of edges when they are filtered
    """
    
    Parameters :
    - distance_matrix_points : distance matrix between points (the goal here is to get dijsktra between each pair of points based on k-nn graph)
    - geodesic_clusters_dm : distance matrix between clusters (for two clusters should be the average distance between points belonging to each of those two clusters. The distance between points should be from the dijsktra between points based on k-nn graph)
    """
    def distortion_geo_evolution(self,distance_matrix_points, geodesic_clusters_dm, maxi , funct = np.nanmax ) :
        evolution_distortion = []
        indexes = list(range(len(self.X)))

        # List of clusters will be on the same order than the AbstractGraph
        nb_clusters = len(self.clusters)
        g = nx.Graph()
       
        # ADD THE VERTICES TO THE GRAPH
        for vertice in self.vertices :
            g.add_node(vertice,**self.graph.nodes(data=True)[vertice]  )

        evolution_distortion.append(maxi)
        # FOR EACH EDGE ADDED WE UPDATE THE DISTORTION MATRIX
        for i in range(len(self.edges)) :
            g.add_edge(self.edges[i][0], self.edges[i][1], label= self.edges[i][2])
            # UPDATE THE DISTORTION MATRIX
            distortion_matrix = distortion_one_graph(self.X, g, distance_matrix_points)


            ratio_matrix = ratio_matrices(distortion_matrix,  geodesic_clusters_dm, maxi)
            d = funct(ratio_matrix )
            evolution_distortion.append(d)

        return evolution_distortion
