...
class ClusterGraph:
    ...
    def my_graph_group_clusters(self, list_groups, nb_edges=-1, edges=None):
        ...
        for i in range(len(list_groups)):
            ...
            for j in range(len(list_groups[i])):
                ...
                points_covered.extend(
                    self.graph.nodes[list_groups[i][j]]["points_covered"]
                )

            self.my_graph.add_node(
                i + 1,
                size=len(points_covered),
        ...
        (
            corres_edges,
            distances_group,
        ) = good_edges_groups(list_groups, self.edges)
    ...
    def distortion_geo_evolution(
        self, distance_matrix_points, geodesic_clusters_dm, maxi, funct=np.nanmax
    ):
